package ru.ivanov.ecommerceplatformproject.productservice.security.filter;
...
import ru.ivanov.ecommerceplatformproject.productservice.util.JWTUtils;
...
public class JWTFilter extends OncePerRequestFilter {
    ...
    private Authentication createUsernamePasswordAuthentication(Claims claims) {
        ...
        List<String> principalRoles = claims.get("roles", List.class);
        List<GrantedAuthority> principalAuthorities = principalRoles.stream()
                .map(SimpleGrantedAuthority::new)
                .collect(Collectors.toList());

        UserDetails userDetails = new JwtPrincipal(principalId, null, principalAuthorities);

        return new UsernamePasswordAuthenticationToken(userDetails, null, principalAuthorities);
    }
